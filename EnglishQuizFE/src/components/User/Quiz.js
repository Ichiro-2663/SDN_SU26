...
const Quiz = () => {
  ...
  if (submitted) {
    ...
    return (
      <div className="p-4 w-100">
        <Button variant="outline-warning" className="mb-3 text-dark fw-bold" onClick={() => { setSelectedExam(null); setSubmitted(false); }}>
  ...
  // --- EXAM TAKING VIEW ---
  if (selectedExam) {
    if (questions.length === 0) return <p>No questions found in this quiz.</p>;
    ...
    return (
      <div className="p-4 w-100">
        ...
        <Card className="p-4 shadow-sm border-warning">
          <div className="d-flex justify-content-between align-items-center mb-3">
             <h5>Question {current + 1} / {questions.length} <Badge bg="info" className="ms-2">{q.questionType}</Badge></h5>
             <Badge bg="warning" text="dark">Duration: {selectedExam.duration} mins</Badge>
          </div>

          {q.passage && (
            <div className="bg-light p-3 rounded mb-3 small" style={{ maxHeight: "150px", overflowY: "auto", borderLeft: "4px solid #f97316" }}>
          ...
          {q.audioUrl && (
            <div className="bg-light p-3 rounded mb-3 d-flex align-items-center gap-2">
              <Button size="sm" variant="warning" onClick={() => handleTTS(q.content)}>
  ...
  // --- LISTING VIEW ---
  const displayedExams = exams.filter(e => filterLevel === "All" || e.level === filterLevel);
    ...
    <div className="w-100 p-2">
      ...
      <Row>
        {displayedExams.length === 0 ? (
          <div className="text-center text-muted my-4">No quizzes available for {filterLevel} level</div>
        ) : (
          displayedExams.map((exam) => (
            <Col key={exam._id} xs={12} sm={6} md={4} lg={3} className="mb-3">
              <Card className="shadow-sm text-center border-warning h-100" style={{ borderRadius: "12px" }}>
                <Card.Body className="d-flex flex-column justify-content-between">
                  <div>
                    <h6 className="fw-bold mb-2">{exam.title}</h6>
                    <Badge bg="info" className="mb-2 me-1">{exam.level || "Mixed"}</Badge>
                    <Badge bg="secondary" className="mb-2">{exam.questions?.length || 0} Questions</Badge>
