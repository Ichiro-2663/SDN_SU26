...
const QuestionBank = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.questionType === "MultipleChoice") {
      ...
      const pairs = [];
      if (formData.pair1Left.trim())
        pairs.push({ left: formData.pair1Left, right: formData.pair1Right });
      if (formData.pair2Left.trim())
        pairs.push({ left: formData.pair2Left, right: formData.pair2Right });
      if (formData.pair3Left.trim())
        pairs.push({ left: formData.pair3Left, right: formData.pair3Right });
  ...
  const filteredQuestions = questions.filter((q) => {
    ...
    if (filters.topic !== "All" && (q.topic?._id || q.topic) !== filters.topic)
      return false;
    if (filters.skill !== "All" && (q.skill?._id || q.skill) !== filters.skill)
      return false;
    return true;
  });
          ...
          {currentQuestions.map((q, idx) => (
            <tr key={q._id}>
              ...
              <td style={{ fontSize: "12px" }}>
                <div>
                  <strong>Topic:</strong> {q.topic?.name || "N/A"}
                </div>
                <div>
                  <strong>Skill:</strong> {q.skill?.name || "N/A"}
                </div>
                {q.certificate && (
                  <div>
                    <strong>Cert:</strong> {q.certificate.name}
                  </div>
                )}
              </td>
              <td>
              ...
              >
                {q.questionType === "MultipleChoice"
                  ? q.options?.[q.correctAnswer] || `Index ${q.correctAnswer}`
        ...
        <div className="d-flex justify-content-center mt-4">
          <Pagination>
            ...
            {[...Array(totalPages)].map((_, i) => {
              // Show only a few pages around current page to avoid huge pagination
              if (
                i === 0 ||
                i === totalPages - 1 ||
          ...
          <Form onSubmit={handleSubmit}>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control
                as="textarea"
                rows={2}
                placeholder="Passage context for reading comprehension"
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control
                type="text"
                placeholder="e.g. listening-prompt.mp3"
            ...
            {formData.questionType === "MultipleChoice" && (
              <>
                <h6 className="fw-bold text-primary mt-4 mb-3">
              ...
              <>
                ...
                <Row className="mb-2 align-items-center">
                  <Col md={5}>
                    <Form.Control
                      type="text"
                      placeholder="Left Item 1 (e.g. Cat)"
                      value={formData.pair1Left}
                      onChange={(e) =>
                        setFormData({ ...formData, pair1Left: e.target.value })
                      }
                      required
                    />
                  </Col>
                  ...
                  <Col md={6}>
                    <Form.Control
                      type="text"
                      placeholder="Right Description 1 (e.g. Con mèo)"
                      value={formData.pair1Right}
                      onChange={(e) =>
                        setFormData({ ...formData, pair1Right: e.target.value })
                      }
                      required
                    />
                  </Col>
                ...
                <Row className="mb-2 align-items-center">
                  <Col md={5}>
                    <Form.Control
                      type="text"
                      placeholder="Left Item 2 (e.g. Dog)"
                      value={formData.pair2Left}
                      onChange={(e) =>
                        setFormData({ ...formData, pair2Left: e.target.value })
                      }
                    />
                  </Col>
                  <Col md={1} className="text-center">
                  ...
                  <Col md={6}>
                    <Form.Control
                      type="text"
                      placeholder="Right Description 2 (e.g. Con chó)"
                      value={formData.pair2Right}
                      onChange={(e) =>
                        setFormData({ ...formData, pair2Right: e.target.value })
                      }
                    />
                  </Col>
                </Row>
                <Row className="mb-2 align-items-center">
                  <Col md={5}>
                    <Form.Control
                      type="text"
                      placeholder="Left Item 3 (e.g. Bird)"
                      value={formData.pair3Left}
                      onChange={(e) =>
                        setFormData({ ...formData, pair3Left: e.target.value })
                      }
                    />
                  </Col>
                  <Col md={1} className="text-center">
                  ...
                  <Col md={6}>
                    <Form.Control
                      type="text"
                      placeholder="Right Description 3 (e.g. Con chim)"
                      value={formData.pair3Right}
                      onChange={(e) =>
                        setFormData({ ...formData, pair3Right: e.target.value })
                      }
                    />
                  </Col>
                </Row>
